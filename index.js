...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: HENRIQUE BOTS\n'
            + 'ORG:Owner HENRIQUE;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5527998401752:+55 27 99840-1752\n'
            + 'END:VCARD'

prefix = "#"
name = "HENRIQUE BOTS OFC"
rdaftar = "OBRIGADO POR SE REGISTAR NO BOT DO HENRIQUE BOTS Wa.me/5527998401752😁"
rmenu = "HENRIQUE BOTS 27998401752:)"
limitt = 50
ban = []
userpremium = ["5527998401752@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `IAE MENBRO NOVO   @${num.split('@')[0]}\nbem vindo (a) ao melhor grupo*${mdata.subject}\nSEU NOME\nSUA IDADE\nVC E MENINA OU MENINO se for menina chame meu criador pfv Wa.me/5527998401752\nNUMERO DO CARTÃO DE CRÉDITO 😁\ndigite #menu para saber mais\ncontato do meu criador\nWa.me/5527998401752*`
				...
				teks = `xauu seu fdp  não foi um prazer te ver aqui  @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do HENRIQUE BOTS digite ${prefix}daftar`,
			...
			const ownerNumber = ["5527998401752@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do HENRIQUE BOTS:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium HENRIQUE BOTS``` :\n'
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('5527998401752@s.whatsapp.net', options, text, {quoted: mek})
                ...
                case 'info':
	        	if (args.length < 1) return reply ('OLA EU SOU UM BOT DE WHATSAPP\ndono do bot Wa.me/5527998401752\ncriado em 30/09/2020\nadiciona meu dono ai pfv')
	            	break
                case 'menu':
		        if (args.length < 1) return reply ('APROVEITA E ADICIONA MEU DONO AI Wa.me/5527998401752')
		            break
                case 'link':
	         	if (args.length < 1) return reply ('GERAL ENTRA NESSE GRUPO RAPIDO\nhttps://chat.whatsapp.com/H8SC0vxpJNgADsuKknWmmu\nhttps://chat.whatsapp.com/D6pzWbHSxnPFImv410f2p3')
	            	break
				case 'ssweb':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ marcando todo mundo fds *${pushname2}* ✪══`+ teks +'╚═══〘 HENRIQUE BOTS 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sucesso deleta todo o chat HENRIQUE BOTS\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *HENRIQUE BOTS 27998401752* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[HENRIQUE BOTS]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
